...
/**
 * Import script for Walmart Media Library page
 * Source: https://corporate.walmart.com/news/media-library
 * Target: /wm-media-library-eds
 *
 * Page Structure:
 * - Search bar + filter tabs (SPA component)
 * - Images section (3942 items, 6 preview cards)
 * - Galleries section (5 items with asset counts)
 * - Videos section (41 items, Vimeo-hosted)
 * - Footer (experience fragment)
 */

/**
 * Wait for the media library content to be rendered
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    const interval = setInterval(() => {
      ...
      if (attempts >= maxAttempts) {
        clearInterval(interval);
        // Resolve anyway - we'll work with what we have
        resolve();
      }
    }, 500);
  });
...
/**
 * Build the image cards block (Cards-Media)
 */
...
/**
 * Build the gallery cards block (Cards-Gallery)
 */
...
/**
 * Known video data - captured from the live page as fallback
 * since video content loads asynchronously via JS
 */
const KNOWN_VIDEOS = [
  {
...
/**
 * Build the video cards block (Cards-Video)
 * Each video card includes a Vimeo link for playback
 * Falls back to known video data if DOM content hasn't loaded
 */
function buildVideoCards(document) {
  const videoContainer = document.querySelector('.videoContainer');
  const cards = videoContainer
    ? videoContainer.querySelectorAll('.corporatesearch-gallery-cards')
    : [];

  // Use live DOM data if available, otherwise fallback to known data
  if (cards.length > 0) {
...
/**
 * Build standalone embed blocks for videos that have IDs
 */
function buildVideoEmbeds(document) {
  const videoContainer = document.querySelector('.videoContainer');
  if (!videoContainer) return [];

  const embeds = [];
  const cards = videoContainer.querySelectorAll('.corporatesearch-gallery-cards');
  cards.forEach((card) => {
    const videoIdEl = card.querySelector('[data-video-id]');
    if (!videoIdEl) return;
    ...
    // Add thumbnail as placeholder image
    if (img) {
      const newImg = document.createElement('img');
      newImg.src = img.src;
      newImg.alt = desc ? desc.textContent.trim() : '';
    ...
    const link = document.createElement('a');
    link.href = 'https://vimeo.com/' + videoId;
    link.textContent = 'https://vimeo.com/' + videoId;
    contentCell.appendChild(link);

    cells.push([contentCell]);

    embeds.push({
      title: desc ? desc.textContent.trim() : 'Video ' + videoId,
      block: WebImporter.Blocks.createBlock(document, { name: 'Embed', cells }),
    });
  });
...
export default {
  onLoad: async ({ document }) => {
    // Wait for the dynamic media library content to render
    try {
      await waitForMediaLibrary(document);
  ...
  transform: ({ document, url }) => {
    ...
    // --- Section 1: Page title and intro ---
    ...
    introP.textContent = introText
      ? introText.textContent.trim()
      : 'Looking for a specific asset? Enter a keyword to refine your results.';
    main.appendChild(introP);

    // Note about interactive search functionality
